...
var AuthenticationService = /** @class */ (function () {
    ...
    AuthenticationService.prototype.getlocation = function () {
        if (!this.location) {
            this.location = localStorage.getItem('mean-location');
        }
        return this.location;
    };
    AuthenticationService.prototype.saveCityManager = function (cityManager) {
        localStorage.setItem('mean-cityManager', cityManager);
        this.cityManager = cityManager;
    };
    AuthenticationService.prototype.isCityManager = function () {
    ...
    AuthenticationService.prototype.request = function (method, type, template) {
        ...
        var request = base.pipe(Object(__WEBPACK_IMPORTED_MODULE_2_rxjs_operators_map__["a" /* map */])(function (data) {
            if (data.token) {
                ...
                _this.saveLocation(data.location);
                if (data.cityManager == 1)
                    _this.saveCityManager("yes");
                else
                    _this.saveCityManager(null);
            }
            return data;
        }));
        return request;
    ...
    // For Message
    AuthenticationService.prototype.sendMessage = function (msg) {
...
var InventoryComponent = /** @class */ (function () {
    ...
    InventoryComponent.prototype.onSubLocationChange = function (id) {
        var resultInv = this.newRRInventories;
    ...
    InventoryComponent.prototype.delModal = function (id) {
        this.inventoryId = id;
        console.log(this.inventoryId);
    };
    InventoryComponent.prototype.deleteLead = function () {
        var id = this.inventoryId;
        for (var i = 0; i < this.inventories.length; i++) {
            if (this.inventories[i].leadId == id) {
...
var AddLeadComponent = /** @class */ (function () {
    ...
    // For sending message
    AddLeadComponent.prototype.sendMessage = function () {
        var lead = this.lead;
        ...
        console.log(msg);
        this.auth.sendMessage(msg).subscribe(function () {
            console.log("success");
        }, function (err) {
            console.error(err);
        });
    ...
    // Construct Msg
    AddLeadComponent.prototype.contructMsg = function (l) {
        // Client name
        var clientName = l.clientName;
        // Purpose
        var purpose = "";
        if (l.purpose == 1)
            purpose = "sell";
        ...
        if (l.purpose == 3)
            purpose = "rent";
        // Property Type
        var type = this.propTypes.filter(function (type) {
            return type._id == l.propTypeId;
        });
        type = type[0].type;
        // Sub-Location
        var subLocation = this.sublocations.filter(function (sloc) {
            return sloc._id == l.sublocationId;
        });
        subLocation = subLocation[0].sublocation;
        // Location
        var location = this.locations.filter(function (loc) {
        ...
        var demand = l.demand;
        // Property Number
        var propNo = l.propNumber;
        // Street
        var street = l.street;
        // Comment
        var cmt = l.cmt;
        // phone
        var phone = l.phoneNumber;
        var msg = "You have assigned a new lead.\n" + clientName + " want to " + purpose + " the " + type
...
var LeadsComponent = /** @class */ (function () {
    function LeadsComponent(auth, router, leadService) {
        ...
        this.router = router;
        this.leadService = leadService;
        this.dtOptions = { order: [[0, "desc"]], responsive: true, autoWidth: false, iDisplayLength: 50 };
        this.dtTrigger = new __WEBPACK_IMPORTED_MODULE_5_rxjs_Subject__["a" /* Subject */]();
        this.isAgent = false;
    ...
    // For table  
    LeadsComponent.prototype.searchTable = function (value) {
        this.datatableElement.dtInstance.then(function (dtInstance) {
            dtInstance.columns(1).search(value).draw();
        });
    };
    LeadsComponent.prototype.searchAgentTable = function (value) {
        this.datatableElement.dtInstance.then(function (dtInstance) {
            dtInstance.columns(4).search(value).draw();
        });
    };
    LeadsComponent.prototype.drawTable = function () {
        this.dtTrigger.next();
        this.datatableElement.dtInstance.then(function (dtInstance) {
            dtInstance.columns().every(function () {
                var that = this;
    ...
    LeadsComponent.prototype.redrawTable = function () {
        var _this = this;
        this.datatableElement.dtInstance.then(function (dtInstance) {
            dtInstance.destroy();
            _this.drawTable();
    ...
    LeadsComponent.prototype.getLeads = function () {
        var _this = this;
        this.auth.getLeads().subscribe(function (leads) {
            _this.applyLeadsFilter(leads);
            _this.resultLeads = _this.setLeads(_this.leads);
            _this.redrawTable();
            _this.leadService.setLeads(_this.leads);
            _this.loading = false;
        });
    };
    LeadsComponent.prototype.applyLeadsFilter = function (leads) {
        var rawleads = leads.filter(function (leads) { return leads.assignedTo !== "0"; });
    ...
    LeadsComponent.prototype.setLeads = function (leads) {
        if (this.auth.isCityManager() !== 'yes')
            return leads;
        this.isCityManager = true;
        var loc = this.auth.getlocation();
        this.inventories = this.inventories.filter(function (inv) { return inv.cityId == loc; });
        var newLeads = [];
        for (var i = 0; i < leads.length; i++)
            for (var j = 0; j < this.inventories.length; j++)
                if (leads[i]._id == this.inventories[j].leadId)
    ...
    LeadsComponent.prototype.onCityChange = function () {
        var cityId = this.selectedCity;
        if (cityId == 0)
            this.searchTable("");
        else {
            var city = this.cities.filter(function (city) { return city._id == cityId; });
    ...
    LeadsComponent.prototype.onAgentChange = function () {
        var agentId = this.selectedAgent;
        if (agentId == 0)
            this.searchAgentTable("");
        else {
            var agent = this.users.filter(function (user) { return user._id == agentId; });
            var agentName = agent[0].name;
    ...
    LeadsComponent.prototype.delModal = function (id) {
        this.leadId = id;
    };
    LeadsComponent.prototype.deleteLead = function () {
        var id = this.leadId;
        for (var i = 0; i < this.leads.length; i++) {
            if (this.leads[i]._id == id) {
...
var OptionsComponent = /** @class */ (function () {
    ...
    OptionsComponent.prototype.getLocations = function () {
        var _this = this;
        var curCity = this.selectedLocCity;
        this.auth.getLocations().subscribe(function (locations) {
            _this.locations = locations;
            if (curCity !== undefined)
                _this.onChange(curCity);
            else
                _this.onChange(_this.auth.getlocation());
        }, function (err) {
            console.error(err);
        });
    };
    ...
    OptionsComponent.prototype.editLoc = function () {
        ...
        this.auth.updateLocation(location).subscribe(function () {
            ...
            _this.getLocations();
            _this.getSubLocations();
            _this.onChange(_this.location.cityId);
        }, function (err) {
            console.error(err);
        });
        this.editLocClicked = false;
...
module.exports = "<div class=\"container\">\n\n  <div class=\"row\">\n    <div class=\"col-md-6\" class=\"mt-4\">\n      <form (submit)=\"updateUser()\">\n        <div class=\"form-group\">\n          <label for=\"name\">Full name</label>\n          <input type=\"text\" class=\"form-control\" name=\"name\" placeholder=\"Enter name\" [(ngModel)]=\"credentials.name\">\n        </div>\n        <div class=\"form-group\">\n          <label for=\"email\">Email address</label>\n          <input type=\"email\" class=\"form-control\" name=\"email\" placeholder=\"Enter email\" [(ngModel)]=\"credentials.email\">\n        </div>\n        <div class=\"form-group\">\n          <label for=\"contact\">Contact #</label>\n          <input type=\"text\" class=\"form-control\" name=\"contact\" placeholder=\"Enter phone number\" [(ngModel)]=\"credentials.phone\">\n        </div>\n        <div class=\"form-group\" *ngIf=\"!isCityManager\">\n          <label>Select City</label>\n          <select class=\"form-control\" id=\"selectCity\" name=\"cityId\" (change)=\"onChange($event.target.value)\" [(ngModel)]=\"credentials.city\">\n            <option *ngFor=\"let city of cities\" value=\"{{ city?._id }}\">{{ city?.name }}</option>\n          </select>\n        </div>\n        <div class=\"form-group\">\n          <label>Select Location</label>\n          <select class=\"form-control\" id=\"selectLocation\" name=\"locId\" [(ngModel)]=\"credentials.location\">\n            <option *ngFor=\"let loc of newLocations\" value=\"{{ loc?._id }}\">{{ loc?.location }}</option>\n          </select>\n        </div>\n        <div class=\"form-group\" *ngIf=\"!isCityManager\">\n          <p>User Access</p>\n          <select class=\"form-control\" id=\"selectAccess\" name=\"access\" [(ngModel)]=\"credentials.access\" (change)=\"onAdminChange()\">\n            <option value=\"1\">Admin</option>\n            <option value=\"2\">Agent</option>\n          </select>\n        </div>\n        <div class=\"form-group\" *ngIf=\"!agentSelected\">\n          <p>City Manager</p>\n          <select class=\"form-control\" id=\"selectAccess\" name=\"cityManager\" [(ngModel)]=\"credentials.cityManager\">\n            <option value=\"1\">Yes</option>\n            <option value=\"0\">No</option>\n          </select>\n        </div>\n        <div class=\"form-group\">\n          <p>Status</p>\n          <select class=\"form-control\" id=\"selectAccess\" name=\"status\" [(ngModel)]=\"credentials.status\">\n            <option value=\"1\">Active</option>\n            <option value=\"2\">Inactive</option>\n          </select>\n        </div>\n\n        <button type=\"submit\" class=\"btn btn-default\">Update User!</button>\n      </form>\n    </div>\n  </div>\n\n</div>"
...
var EditUserComponent = /** @class */ (function () {
    ...
    EditUserComponent.prototype.getCities = function () {
        var _this = this;
        this.auth.getCities().subscribe(function (cities) {
            _this.cities = cities;
        }, function (err) {
    ...
    EditUserComponent.prototype.onChange = function (id) {
        this.newLocations = this.locations.filter(function (locations) {
            return locations.cityId == id;
        });
    };
    EditUserComponent.prototype.getLocations = function () {
        var _this = this;
        this.auth.getLocations().subscribe(function (locations) {
            _this.locations = locations;
            _this.onChange(_this.credentials.city);
        }, function (err) {
            console.error(err);
        });
    };
    EditUserComponent.prototype.onAdminChange = function () {
...
module.exports = "<div class=\"container\">\n\n  <div class=\"row\">\n    <div class=\"col-md-6\" class=\"mt-4\">\n      <form (submit)=\"register()\">\n        <div class=\"form-group\">\n          <label for=\"name\">Full name</label>\n          <input type=\"text\" class=\"form-control\" name=\"name\" placeholder=\"Enter name\" [(ngModel)]=\"credentials.name\">\n        </div>\n        <div class=\"form-group\">\n          <label for=\"email\">Email address</label>\n          <input type=\"email\" class=\"form-control\" name=\"email\" placeholder=\"Enter email\" [(ngModel)]=\"credentials.email\">\n        </div>\n        <div class=\"form-group\">\n          <label for=\"contact\">Contact #</label>\n          <input type=\"text\" class=\"form-control\" name=\"contact\" placeholder=\"Enter phone number\" [(ngModel)]=\"credentials.phone\">\n        </div>\n        <div class=\"form-group\" *ngIf=\"!isCityManager\">\n          <label>Select City</label>\n          <select class=\"form-control\" id=\"selectCity\" name=\"cityId\" (change)=\"onChange($event.target.value)\" [(ngModel)]=\"credentials.city\">\n            <option *ngFor=\"let city of cities\" value=\"{{ city?._id }}\">{{ city?.name }}</option>\n          </select>\n        </div>\n        <div class=\"form-group\">\n          <label>Select Location</label>\n          <select class=\"form-control\" id=\"selectLocation\" name=\"locId\" [(ngModel)]=\"credentials.location\">\n            <option *ngFor=\"let loc of newLocations\" value=\"{{ loc?._id }}\">{{ loc?.location }}</option>\n          </select>\n        </div>\n        <div class=\"form-group\">\n          <label for=\"password\">Password</label>\n          <input type=\"password\" class=\"form-control\" name=\"password\" placeholder=\"Password\" [(ngModel)]=\"credentials.password\">\n        </div>\n        <div class=\"form-group\" *ngIf=\"!isCityManager\">\n          <p>User Access</p>\n          <div class=\"form-check form-check-inline\">\n            <input class=\"form-check-input\" type=\"radio\" [(ngModel)]=\"credentials.access\" name=\"access\" id=\"access1\" value=\"1\" (change)=\"onAdminChange()\">\n            <label class=\"form-check-label\" for=\"access\">Admin</label>\n          </div>\n          <div class=\"form-check form-check-inline\">\n            <input class=\"form-check-input\" type=\"radio\" [(ngModel)]=\"credentials.access\" name=\"access\" id=\"access2\" value=\"2\" (change)=\"onAgentChange()\">\n            <label class=\"form-check-label\" for=\"access\">Agent</label>\n          </div>\n        </div>\n        <div class=\"form-group\" *ngIf=\"!agentSelected\">\n          <p>City Manager?</p>\n          <div class=\"form-check form-check-inline\">\n            <input class=\"form-check-input\" type=\"radio\" [(ngModel)]=\"credentials.cityManager\" name=\"cityManager\" id=\"cityManager1\" value=\"1\">\n            <label class=\"form-check-label\" for=\"cityManager\">Yes</label>\n          </div>\n          <div class=\"form-check form-check-inline\">\n            <input class=\"form-check-input\" type=\"radio\" [(ngModel)]=\"credentials.cityManager\" name=\"CityManager\" id=\"cityManager2\" value=\"0\">\n            <label class=\"form-check-label\" for=\"cityManager\">No</label>\n          </div>\n        </div>\n        <div class=\"form-group\">\n          <p>Status</p>\n          <div class=\"form-check form-check-inline\">\n            <input class=\"form-check-input\" type=\"radio\" [(ngModel)]=\"credentials.status\" name=\"status\" id=\"status1\" value=\"1\">\n            <label class=\"form-check-label\" for=\"status\">Active</label>\n          </div>\n          <div class=\"form-check form-check-inline\">\n            <input class=\"form-check-input\" type=\"radio\" [(ngModel)]=\"credentials.status\" name=\"status\" id=\"status2\" value=\"2\">\n            <label class=\"form-check-label\" for=\"status\">Inactive</label>\n          </div>\n        </div>\n\n        <button type=\"submit\" class=\"btn btn-default\">Add User!</button>\n      </form>\n    </div>\n  </div>\n\n</div>"
...
var RegisterComponent = /** @class */ (function () {
    ...
    RegisterComponent.prototype.onChange = function (id) {
        this.newLocations = this.locations.filter(function (locations) {
            return locations.cityId == id;
        });
    };
    RegisterComponent.prototype.getLocations = function () {
        var _this = this;
        this.auth.getLocations().subscribe(function (locations) {
            _this.locations = locations;
            _this.onChange(_this.credentials.city);
    ...
    RegisterComponent.prototype.onAgentChange = function () {
        if (this.credentials.access == 2)
            this.agentSelected = true;
    };
    RegisterComponent.prototype.onAdminChange = function () {
        if (this.credentials.access == 1)
            this.agentSelected = false;
    };
...
var UsersComponent = /** @class */ (function () {
    ...
    UsersComponent.prototype.delModel = function (userId) {
    ...
    UsersComponent.prototype.deleteUser = function () {
        ...
        this.auth.deleteUser(user).subscribe(function () {
            console.log("success");
            _this.getUsers();
        }, function (err) {
            console.error(err);
    ...
    UsersComponent.prototype.getCities = function () {
        var _this = this;
        this.auth.getCities().subscribe(function (cities) {
            _this.cities = cities;
        }, function (err) {
    ...
    UsersComponent.prototype.getLocations = function () {
        var _this = this;
        this.auth.getLocations().subscribe(function (locations) {
            _this.locations = locations;
        }, function (err) {
    ...
    UsersComponent.prototype.getCity = function (id) {
        var city = this.cities.filter(function (city) {
            return city._id == id;
        });
        return city;
    };
    UsersComponent.prototype.getLocation = function (id) {
